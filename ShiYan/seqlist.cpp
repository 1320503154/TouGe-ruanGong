...
// 堆排序，在函数中输出每趟排序后的结果
// 本关任务：
//  以线性表L中的课程名称信息作为关键字，完成建堆、排序调整及输出排序结果的函数。
//  输出每趟排序后的课程名称。最后输出排序完成后的线性表内容。
// 建堆
// 给我一个可视化演示的网站:https://www.cs.usfca.edu/~galles/visualization/HeapSort.html
...
// 构建堆的函数，以最大堆为例
void buildHeap(PseqList &L)
{
    int n = L->curNum;                   // 获取线性表的元素个数
    for (int i = n / 2 - 1; i >= 0; i--) // 从最后一个非叶子节点开始，从下往上进行调整
    {
        int k = i;         // 将当前位置设为k
        int j = 2 * k + 1; // 设定k的左子节点位置j，右子节点位置为j + 1 = 2 * k + 2
        while (j < n)      // 当j小于线性表的元素个数时，循环执行以下操作
        {
            // 如果右子节点存在并且右子节点的课程名称大于左子节点的课程名称
            if (j + 1 < n && L->element[j].coursename < L->element[j + 1].coursename)
            {
                j++; // 将右子节点的位置设为j
            }
            // 如果k位置的元素的课程名称小于j位置的元素的课程名称
            if (L->element[k].coursename < L->element[j].coursename)
            {
                swapData(L->element[k], L->element[j]); // 交换元素
                // k是父节点，j是子节点，交换后，k位置的元素的课程名称大于等于j位置的元素的课程名称
                k = j;
                j = 2 * k + 1;
            }
            else
            {
                break; // 如果k位置的元素的课程名称大于等于j位置的元素的课程名称，则跳出循环
            }
        }
    }
}
...
void heap_sort(PseqList &L)
{
    ...
    for (int i = n - 1; i > 0; i--)
    {
        swapData(L->element[0], L->element[i]); // 交换堆顶元素和最后一个元素
        int k = 0;                              // k是父节点
        int j = 2 * k + 1;                      // j是左子节点
        while (j < i)
        {
            // 如果右子节点存在并且右子节点的课程名称大于左子节点的课程名称
            if (j + 1 < i && L->element[j].coursename < L->element[j + 1].coursename)
            {
                j++;
            }
            // 如果k位置的元素的课程名称小于j位置的元素的课程名称
            if (L->element[k].coursename < L->element[j].coursename)
            {
                swapData(L->element[k], L->element[j]);
                k = j;
                j = 2 * k + 1;
            }
            else
...
void bubble_sort(PseqList &L)
{
    ...
    {
        int swapped = 0; // 用于标记是否发生交换
        for (int j = 0; j < L->curNum - i - 1; j++)
        {
            // 每一趟排序都会把最大的元素放到最后面,所以每一趟排序都会少比较一次.
            // 这也就是为什么j < L->curNum - i - 1,此处的L->curNum - i - 1就是每一趟排序要比较的次数.
            if (L->element[j].coursename > L->element[j + 1].coursename) // 比较相邻的元素,如果前面的元素大于后面的元素,则交换
            {
                // 交换元素
                DataType temp = L->element[j + 1];
                L->element[j + 1] = L->element[j];
                L->element[j] = temp; // L->element[j] = L->element[j + 1];
                swapped = 1;
            }
        }
        if (!swapped) // 如果没有发生交换,说明已经排好序了,直接退出循环
        {
            break;
        }
    }
...
void quick_sort(PseqList &L, int left, int right)
{
    ...
    if (left >= right)
    {
        return; // 若左边索引大于等于右边索引，表示已经完成排序，递归结束
    }

    DataType pivot = L->element[left]; // 选取第一个元素作为基准

    while (left < right)
    {
        // 从右往左找到第一个小于基准值的元素
        while (left < right && L->element[right].coursename >= pivot.coursename)
        {
            right--;
        }
        // 此时right指向的元素的课程名小于基准值
        L->element[left] = L->element[right]; // 将找到的小于基准值的元素放到基准值左边
        left++;

        // 从左往右找到第一个大于等于基准值的元素
        while (left < right && L->element[left].coursename < pivot.coursename)
        {
            left++;
        }

        L->element[right] = L->element[left]; // 将找到的大于等于基准值的元素放到基准值右边
        right--;
    }
    // 当循环结束,left和right指向同一个元素，即基准值所在的位置
    L->element[left] = pivot; // 将基准值放回正确的位置

    printCourse(L);

    // 递归调用快速排序对基准值左右两侧的子数组进行排序
    quick_sort(L, 0, left - 1);
    quick_sort(L, left + 1, L->curNum - 1);
}
